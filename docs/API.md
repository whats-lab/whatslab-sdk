...
| `TorchKeyvectorFK(kv, idx_q, joint_names, dtype=float32)` | **배치 FK 를 순수 torch 로** — 사슬을 URDF 에서 뽑아 회전축을 수치로 유도한 뒤 Rodrigues 로 배치 전개한다. autograd 가 그대로 붙어 수동 야코비안이 필요 없고 CUDA 로 올라간다. `HandKeyvector.encode`/`jacobian`(pinocchio)와 값·기울기가 기계 정밀도로 일치하는 것을 테스트가 고정한다. 학습 전용 — 추론 경로에는 FK 가 없다. |
