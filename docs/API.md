...
| `coverage_loss` / `motion_loss_local` / `pinch_loss` / `position_loss` / `soft_pinch_loss` / `chamfer_both` / `chamfer_partial` | 학습 손실 4종이 정본이다 — `motion_loss_local`(두 섭동의 사잇각, 6D 회전에 불변) + `coverage_loss` + `pinch_loss` + `position_loss`(손가락별 6D keyvector 일치). `coverage_loss(partial=True)` 는 단방향 Chamfer 로 로봇 여분 영역 왜곡을 피한다. `bone_loss`/`distance_loss`/`extension_loss`/`flatness_loss`/`motion_loss_global` 은 제거했다 — 근거는 `plans/RETARGET_V2_PLAN.md`. |
