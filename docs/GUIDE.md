...
1. **정확도는 `--traj fk` 로만 잰다.**
   ```bash
   $PY tools/bench_arm_ik.py --traj fk --floor
   ```
   FK 궤적은 오차 하한이 0 이므로 남는 오차가 전부 솔버 탓이다. 좌표계로 합성한
   궤적은 도달 불가 구간을 지나 솔버 품질과 도달성을 섞는다.
...
3. **데이터셋 2개 × 시작점 여러 개**로 잰다. 단일 시작점 리플레이는 초기 basin
   ...
   에서 89mm, 15% 지점에서 5mm). 합성 궤적에서는 `--seeds` 가 같은 역할을 한다:
   ```bash
   $PY tools/bench_arm_ik.py --traj walk --seeds 40
   ```

`--floor` 는 전역 탐색으로 그 프레임의 **달성 가능 하한**을 낸다. 추종 오차가
하한에 붙어 있으면 솔버는 할 일을 다 한 것이고, 남은 오차는 도달성이나 rig 문제다.
