...
def test_lref_is_max_reachable_dorsum_to_tip_over_all_fingers():
    ...
    for _ in range(200):
        ...
        pts = kv.points(q)
        for f in FINGERS:
            d = float(np.linalg.norm(pts[f][-1] - kv.origin))
            assert d <= kv.l_ref * (1.0 + 1e-6), f
    assert np.abs(kv.encode(pin.neutral(fk.model))[:, :3]).max() <= 1.0 + 1e-9
