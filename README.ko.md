...
**적정 epoch 는 400 이다.** 검증 분할(`--val-frac 0.1`)로 재면 손실이 300~400 에서
정체하고 600 까지 반등이 없다(300→600 이득 1.4~2.3%). 벤치도 같은 지점에서 평평해진다
— LMC 가 100/200/300/**400**/500/600/1000 epoch 에서 82.8/84.5/85.9/**86.2**/86.4/86.3/85.7 이고
말단°는 100 epoch 부터 이미 평평하다. 600 을 넘기면 이득이 없고 `|dq|p95` 와 LMC 가
약간 나빠진다. 더 돌리는 것이 해로운 경우도 있다 — 출력 포화가 깊어져 관절이 죽는다
(dg5f 소지 뿌리가 300epoch 6.5deg/포화87% 에서 1000epoch 0.4deg/포화100%). 오래 돌릴
거면 `--u-margin 1.25` 나 `--w-sat` 을 함께 쓴다.

검증 손실은 **런 내부 epoch 선택에만** 쓴다 — 가중치가 다른 런끼리는 총손실 정의가
달라 비교할 수 없다. 가중치 순위는 벤치로만 정한다.

**추론은 `torch.set_num_threads(1)` 로 고정한다.** batch 1 / 128폭 짜리 워크로드라
멀티스레딩이 순손실이고, 기본값(코어 수)에서는 함정이 있다 — 측정: 32스레드에서
LeakyReLU 망 0.200ms / **GELU 망 60.4ms**, 1스레드에서 0.100ms / 0.143ms 다.
`nn.GELU` 가 작은 텐서에 병렬 영역을 열어 420배 오버헤드를 낸다. 라이브러리는
프로세스 전역 설정을 건드리지 않으니 소비자가 직접 불러야 한다
(`whatslab.solvers.hand.net_retargeter.INFER_THREADS` 가 권장값이다).
