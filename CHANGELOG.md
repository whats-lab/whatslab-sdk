...
- **`KPHandRetargeter`** (`solvers/hand/kp_retargeter.py`) — 손 리타게팅의 `kp`
  ...
  - **dex-retargeting vector 방법론 2건은 측정으로 기각.** ① 사슬에 **팜→너클**
    ...
    그건 처음부터 들어가 있다.
  - **`scale_mode`**(`uniform` 기본 / `per_finger`) — 구조 차이 중 **뼈 길이 비율**을
    어떻게 다룰지 고르는 손잡이. 사람과 로봇의 손가락 비율이 다르다(실측: orca 엄지가
    스케일된 사람 엄지의 **0.71**, robotis 는 손가락 4개가 전부 95.8mm 인데 사람은
    74~104mm 라 소지 비율이 **1.29**). `uniform` 은 공간상 접촉 관계를 보존하고,
    `per_finger` 는 손가락마다 뼈합을 맞춰 목표를 도달 가능하게 만든다:

    | | 자기목표 지문 | 형상 | 접촉오차 |
    |---|---|---|---|
    | orca `uniform` | 6.3mm | 33.3° | +6.7mm |
    | orca `per_finger` | **3.8mm** | **32.2°** | **+1.3mm** |
    | robotis `uniform` | 23.2mm | 34.6° | **+30.8mm** |
    | robotis `per_finger` | 22.8mm | 35.3° | +36.8mm |

    orca 는 전부 개선(엄지가 짧아 균일 스케일이 도달 불가 목표를 만들던 것),
    robotis 는 무의미하다(손가락이 다 같은 길이라 비율을 맞출 게 없다).
    **스케일로는 못 고치는 것도 확인했다** — robotis 지문 오차는 스케일을
    ×0.75~×1.10 훑어도 29.4/26.6/24.1/23.2/25.3mm 로 유도값이 이미 최적이고,
    지문전용 다중시드 하한이 21.2mm 다. 즉 남은 것은 DOF·비율 구조 한계다.
